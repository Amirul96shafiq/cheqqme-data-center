                    ...
                    ->schema([
                        Infolists\Components\TextEntry::make('client_ids')
                            ...
                            ->formatStateUsing(function ($state, $record) {
                                ...
                                $clients = Client::withTrashed()->whereIn('id', $clientIds)->get(['id', 'pic_name']);

                                $links = $clients->map(function ($client) {
                                    $url = $client->trashed() ? null : route('filament.admin.resources.clients.edit', $client->id);

                                    return $url ? '<a href="'.$url.'" class="text-primary-600 hover:text-primary-700 underline" target="_blank" rel="noopener noreferrer">'.e($client->pic_name).'</a>' : e($client->pic_name);
                                });
                        ...
                        Infolists\Components\TextEntry::make('document_ids')
                            ...
                            ->formatStateUsing(function ($state, $record) {
                                ...
                                $documents = Document::withTrashed()->whereIn('id', $documentIds)->get(['id', 'title', 'type', 'file_path', 'url']);
