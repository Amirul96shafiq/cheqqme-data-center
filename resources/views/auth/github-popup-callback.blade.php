...
<body>
    ...
    <script>
        ...
        if (error) {
            ...
            // Process OAuth code
            fetch('/auth/github/callback?' + window.location.search, {
                method: 'GET',
                headers: {
                    ...
                    try {
                        ...
                        if (window.opener && !window.opener.closed) {
                            if (isFromProfile) {
                                ...
                                sendErrorToParent(errorData.message || 'Failed to authenticate with GitHub. Please try again.');
                            }
                        }

                        closePopupWithFallback(errorData.message || 'Failed to authenticate with GitHub. Please try again.', true);
                        return;
                    } catch (parseError) {
            ...
            .catch(error => {
                sendErrorToParent('Failed to authenticate with GitHub. Please try again.');
                closePopupWithFallback('Failed to authenticate with GitHub. Please try again.', true);
            });
        } else {
