...
<!-- Generic Typing Animation Script -->
<script>
/**
 * Generic TypingAnimation class for any text element
 * Usage: new TypingAnimation(element, options)
 */
class TypingAnimation {
    ...
    typeText(text, callback) {
        ...
        const typeNextChar = () => {
            if (index < text.length && !this.isDestroyed) {
                ...
                this.element.innerHTML = currentText + `<span class="${this.config.cursorClass}"></span>`;
                index++;
                
                const speed = this.config.typeSpeed.min + Math.random() * (this.config.typeSpeed.max - this.config.typeSpeed.min);
    ...
    eraseText(callback) {
        ...
        const eraseNextChar = () => {
            if (currentText.length > 0 && !this.isDestroyed) {
                ...
                const speed = this.config.eraseSpeed.min + Math.random() * (this.config.eraseSpeed.max - this.config.eraseSpeed.min);
    ...
    start() {
        ...
        this.intervalId = setInterval(() => this.changeText(), this.config.interval);
